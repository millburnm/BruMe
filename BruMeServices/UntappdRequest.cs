...
namespace BruMeServices
{
    ...
    {
        string GetUntappdUserInfo(string userName);
    }

    public class UntappdRequest : IUntappdRequest
    {
        ...
        public virtual string GetUntappdUserInfo(string userName)
        {
        ...
        ///
        public string GetUntappdUserBeerDistinctBeers(string userName)
        {
            string uri = string.Format("{0}user/beers/{1}?client_id={2}&client_secret={3}", UNTAPPD_URI, userName, CLIENTID, CLIENT_SECRET);
            untappdRequest = WebRequest.Create(uri);
            untappdRequest.Method = "GET";
